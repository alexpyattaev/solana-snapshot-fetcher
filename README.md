...
1. Finds all available RPCs (via RPC by default, if that fails via gossip spy)
...
Mainnet
```python
snapshot-fetcher --snapshot_path $HOME/solana/validator-ledger
``` 
`$HOME/solana/validator-ledger/` - path to your `validator-ledger`

Testnet
```python
snapshot-fetcher --snapshot_path $HOME/solana/validator-ledger -r http://api.testnet.solana.com
``` 
## Update  
